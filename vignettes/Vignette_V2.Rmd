...
Extension can be one of `'png'`, `'eps'`, `'svg'` or `'pdf'`.
* `PlotDirectory`:  
...
* `PlotFile`:  
Name of the output file including the extension one of `'png'`, `'pdf'`, `'eps'`, `'svg'`. (If no extension specified `'png'` is used).
* `PlotDirectory`:  
...
NOTE: Make sure you have permission to write to the path `system.file('extdata', package = 'ORA')` (e.g. execute R as admin).
